...
func TestMessage(t *testing.T) {
	m := NewMessage()
	m.SetRecipient(m.FormatAddress("to@example.com", "Señor To"), "tobis@example.com")
	m.SetAddressHeader("Cc", "cc@example.com", "A, B")
	m.SetSubject("Hello Masbro")
	...
	m.SetBody("text/html", ParseTemplate("_fixture/example.html", data))
	...
	want := &message{
		...
		content: "From: \"System example\" <noreply@example.com>\r\n" +
...
func stubSendMail(t *testing.T, bCount int, want *message) SendFunc {
	return func(from string, to []string, m io.WriterTo) error {
		assert.Equal(t, want.from, from)
		assert.Equal(t, len(to), len(want.to), fmt.Sprintf("Invalid recipient count, \ngot %d: %q\nwant %d: %q",
			len(to), to, len(want.to), want.to))

		for i := range want.to {
			assert.Equal(t, want.to[i], to[i])
		}
